        ...
        <div className="font-display text-primary/90 bg-[#0c0b06] min-h-screen flex justify-center items-center overflow-hidden relative selection:bg-amber-500/30">
            ...
            <div className="w-full max-w-md h-screen relative bg-[#0c0b06]/90 sm:h-[53.125rem] sm:rounded-3xl sm:border sm:border-primary/20 sm:shadow-2xl overflow-hidden flex flex-col backdrop-blur-md">
                    ...
                    <div className="bg-[#110f0a] rounded-xl p-6 relative overflow-hidden border border-primary/10">
                        ...
                        <div className="flex items-center gap-5 relative z-10 mb-6">
                            ...
                            <div className="flex flex-col">
                                <h2 className="text-2xl font-bold text-primary tracking-tight">{profile?.name}</h2>
                        ...
                        <div className="space-y-4 relative z-10">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <span className="text-[0.625rem] uppercase tracking-widest text-primary/40 font-bold block mb-1">Guild ID</span>
                                    <span className="font-mono text-primary/80 text-sm">MER-8821-X</span>
                                </div>
                                <div>
                                    <span className="text-[0.625rem] uppercase tracking-widest text-primary/40 font-bold block mb-1">Rank</span>
                                    <span className="font-mono text-primary/80 text-sm">Silver III</span>
                                </div>
                            </div>
                            ...
                            <div className="pt-2">
                                <div className="flex justify-between items-end mb-2">
                                    <span className="text-xs uppercase tracking-widest text-primary/60 font-bold">Wealth XP</span>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-[#16140d] p-4 rounded-lg border border-primary/10 flex flex-col items-center gap-1">
                                    <span className="text-2xl font-bold text-primary">{profile?.stats.goldEarned.toLocaleString()}</span>
                                    <span className="text-[0.625rem] uppercase tracking-widest text-stone-500">Gold Earned</span>
                                </div>
                                <div className="bg-[#16140d] p-4 rounded-lg border border-primary/10 flex flex-col items-center gap-1">
                                    <span className="text-2xl font-bold text-stone-300">{profile?.stats.questsCompleted}</span>
                                    <span className="text-[0.625rem] uppercase tracking-widest text-stone-500">Quests Complete</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex-1 p-6 space-y-2 overflow-y-auto no-scrollbar">
                    <h3 className="text-xs uppercase tracking-widest text-stone-500 font-bold mb-4 pl-2">System</h3>

                    {[
                        { icon: 'manage_accounts', label: 'Edit Identity' },
                        { icon: 'notifications', label: 'Raven Alerts' },
                        { icon: 'shield', label: 'Security Spells' },
                        { icon: 'language', label: 'Common Tongue' },
                        { icon: 'help', label: 'Ask the Sages' }
                    ].map((item, i) => (
                        <button key={i} className="w-full flex items-center gap-4 p-4 rounded-xl hover:bg-primary/5 border border-transparent hover:border-primary/10 transition-all text-left group">
                            <span className="material-symbols-outlined text-primary/60 group-hover:text-primary transition-colors">{item.icon}</span>
                            <span className="text-stone-300 group-hover:text-primary transition-colors flex-1">{item.label}</span>
                    ...
                    >
                        ...
                        <span className="text-red-400 group-hover:text-red-300 transition-colors flex-1">Abandon Quest</span>
                    </button>
                </div>
