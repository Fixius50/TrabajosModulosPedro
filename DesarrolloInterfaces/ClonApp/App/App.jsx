...
const INITIAL_WORKSPACES = [
    { 
        ...
        pages: [
            { id: 'p-1', title: 'Música (Bandas)', icon: '🎵', cover: 'https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?auto=format&fit=crop&w=1200&q=80', updatedAt: new Date().toISOString(), isPrivate: true, blocks: [{ id: 'b-1', type: 'p', content: 'Lista de bandas por ver...' }] },
            { id: 'p-2', title: 'Viaje a Japón', icon: '✈️', cover: 'https://images.unsplash.com/photo-1493976040374-85c8e12f0c0e?auto=format&fit=crop&w=1200&q=80', updatedAt: new Date().toISOString(), isPrivate: true, blocks: [{ id: 'b-2', type: 'p', content: 'Itinerario de Tokyo...' }] },
            { id: 'p-3', title: 'Proyectos', icon: '💼', cover: null, updatedAt: new Date().toISOString(), isPrivate: true, blocks: [{ id: 'b-3', type: 'p', content: 'Tareas pendientes...' }] }
        ] 
    }
];
...
const BreathingText = ({ text }) => (
    <div className="flex flex-col items-center justify-center gap-4">
        <motion.div
            ...
            className="text-3xl md:text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-center"
        >
            {text}
        </motion.div>
        <motion.p 
    ...
    <AnimatePresence>
        {isVisible && (
            <motion.div
                ...
                className="fixed bottom-6 right-6 bg-zinc-900/90 backdrop-blur-md text-white p-4 rounded-xl shadow-2xl flex items-center gap-4 z-[100] border border-zinc-700/50 max-w-sm"
            >
                <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-2 rounded-full shadow-lg">
                    <Sparkles size={18} className="text-white" />
                </div>
                <div className="flex-1">
                    <h4 className="font-bold text-sm">¡Magia completada!</h4>
                    <p className="text-xs text-zinc-300 line-clamp-2">{message}</p>
                </div>
                <button 
                    onClick={onClose}
                    className="bg-white text-black px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-zinc-200 transition-colors shadow-sm"
                >
                    Ver
                </button>
            </motion.div>
...
const FancyEditable = ({ tagName: Tag, html, className, onChange, onKeyDown, placeholder, disabled }) => {
    ...
    useLayoutEffect(() => {
        if (contentEditableRef.current && contentEditableRef.current.innerText !== html) {
            contentEditableRef.current.innerText = html;
        }
    }, [html]);
    const handleInput = (e) => {
        const newValue = e.currentTarget.innerText;
        if (onChange && newValue !== html) onChange(newValue);
    };
...
const EditorBlock = ({ block, index, updateBlock, addBlock, deleteBlock }) => {
    const handleKeyDown = (e) => {
        if (e.key === 'Enter') { e.preventDefault(); addBlock(index + 1); }
        else if (e.key === 'Backspace' && (!block.content || block.content === '')) { e.preventDefault(); deleteBlock(index); }
    };
    return (
...
export default function App() { // Cambio a exportación por defecto
    ...
    // --- PERSISTENCE ---
    useEffect(() => {
        localStorage.setItem('notion_v38_workspaces', JSON.stringify(workspaces));
    }, [workspaces]);
    ...
    useEffect(() => {
        ...
        script.type = 'module';
        script.innerHTML = `import { Octokit } from "https://esm.sh/@octokit/rest"; window.Octokit = Octokit; window.dispatchEvent(new Event('octokit-loaded'));`;
        document.head.appendChild(script);
    }, []);

    // --- GITHUB FETCH ---
    const fetchMarketData = async () => {
        if (!window.Octokit) await new Promise(resolve => window.addEventListener('octokit-loaded', resolve, { once: true }));
        setIsLoadingMarket(true);
        if (!githubToken) { setIsLoadingMarket(false); return; }

        try {
            const octokit = new window.Octokit({ auth: githubToken });
            ...
            const { data: rootContents } = await octokit.rest.repos.getContent({ owner, repo, path: basePath });
            
            for (const item of rootContents) {
                if (item.type === 'dir') {
                    if (item.name === 'stylessApp' || item.name === 'themesApp') {
                        try {
                            const { data: files } = await octokit.rest.repos.getContent({ owner, repo, path: item.path });
                            for (const f of files) {
                                if (f.name.match(/\.(css|json)$/i)) {
                                    styles.push({ id: f.sha, name: f.name.replace(/\.(css|json)$/, ''), author: 'Fixius50', color: 'bg-zinc-100 text-zinc-800', download_url: f.download_url });
                                }
                            }
                    ...
                    if (item.name === 'tipographyApp') {
                        try {
                            const { data: subfolders } = await octokit.rest.repos.getContent({ owner, repo, path: item.path });
                            for (const sub of subfolders) {
                                if (sub.type === 'dir') {
                                    const { data: fontFiles } = await octokit.rest.repos.getContent({ owner, repo, path: sub.path });
                                    const font = fontFiles.find(f => f.name.match(/\.(ttf|otf)$/i));
                                    if(font) fonts.push({ id: sub.sha, name: sub.name, type: 'font', download_url: font.download_url });
                                }
                            }
                        } catch(e) { console.warn(e); }
                    }
    ...
    const generatePageWithAI = async () => {
        ...
        try {
            ...
            setShowAI(false);
            setAiPrompt("");
            setNotification({ show: true, message: `"${newPage.title}" ha sido añadida a tu espacio.` });
    ...
    // --- CORE ACTIONS ---
    const updatePage = (pageId, updates) => { setWorkspaces(workspaces.map(ws => ws.id === activeWorkspaceId ? { ...ws, pages: ws.pages.map(p => p.id === pageId ? { ...p, ...updates } : p) } : ws)); };
    const createPage = () => { const id = generateId(); const newPage = { id, title: '', icon: null, cover: null, isPrivate: true, updatedAt: new Date().toISOString(), blocks: [{ id: generateId(), type: 'p', content: '' }] }; setWorkspaces(workspaces.map(ws => ws.id === activeWorkspaceId ? { ...ws, pages: [...ws.pages, newPage] } : ws)); setActivePageId(id); setCurrentView('page'); };
    ...
    const updateBlock = (bid, upd) => updatePage(activePageId, { blocks: activePage.blocks.map(b => b.id === bid ? { ...b, ...upd } : b) });
    ...
    const importResource = (item, type) => {
        const newRes = { id: generateId(), name: item.name, type };
        if (type === 'style') setImportedResources(p => ({ ...p, styles: [...p.styles, newRes] }));
        else setImportedResources(p => ({ ...p, fonts: [...p.fonts, newRes] }));
        alert("Recurso importado correctamente.");
    };
    ...
    return (
        <div className="flex h-screen w-full bg-[#FBFBFA] text-zinc-800 font-sans selection:bg-indigo-100 relative">
            ...
            {/* SIDEBAR */}
            <motion.aside initial={false} animate={{ width: sidebarOpen ? 240 : 0, opacity: sidebarOpen ? 1 : 0 }} className="h-full bg-[#F7F7F5] flex flex-col z-20 relative border-r border-zinc-200 overflow-hidden">
                <div className="p-3 relative shrink-0">
                    <div onClick={() => setShowWorkspaceMenu(!showWorkspaceMenu)} className="flex items-center gap-2 p-1.5 rounded hover:bg-zinc-200 transition-colors cursor-pointer overflow-hidden">
                        ...
                        <div className="flex-1 overflow-hidden text-sm font-medium truncate text-zinc-700">{workspace.name}</div>
                        <ChevronDown size={12} className="text-zinc-500"/>
                    </div>
                    <AnimatePresence>
                        {showWorkspaceMenu && (
                            <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 5 }} className="absolute top-12 left-2 right-2 bg-white border border-zinc-200 rounded-lg shadow-lg z-50 overflow-hidden">
                                <div className="p-2 border-b border-zinc-100"><span className="text-[10px] font-medium text-zinc-500 px-2">TUS ESPACIOS</span></div>
                                {workspaces.map(ws => <div key={ws.id} onClick={() => { setActiveWorkspaceId(ws.id); setShowWorkspaceMenu(false); setCurrentView('home'); }} className="flex items-center gap-2 p-2 hover:bg-zinc-100 cursor-pointer text-sm"><div className={`w-5 h-5 rounded bg-gradient-to-br ${ws.color} text-white flex items-center justify-center text-[10px]`}>{ws.initial}</div><span className="flex-1">{ws.name}</span>{ws.id === activeWorkspaceId && <Check size={14} className="text-indigo-600"/>}</div>)}
                            </motion.div>
                        )}
                    </AnimatePresence>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-2 space-y-0.5 fancy-scroll">
                    <div onClick={() => setShowSearch(true)} className="flex items-center gap-3 p-1.5 rounded hover:bg-zinc-200 text-zinc-600 cursor-pointer"><Search size={16} /> <span className="text-sm">Buscar</span></div>
                    <div onClick={() => { setCurrentView('home'); setActivePageId(null); }} className={`flex items-center gap-3 p-1.5 rounded hover:bg-zinc-200 cursor-pointer ${currentView === 'home' ? 'bg-zinc-200 text-zinc-900 font-medium' : 'text-zinc-600'}`}><SidebarIcon size={16} /> <span className="text-sm">Inicio</span></div>
                    <div className="flex items-center gap-3 p-1.5 rounded hover:bg-zinc-200 text-zinc-600 cursor-pointer"><Users size={16} /> <span className="text-sm">Reuniones</span></div>
                    <div onClick={() => setShowAI(true)} className="flex items-center gap-3 p-1.5 rounded hover:bg-zinc-200 text-zinc-600 cursor-pointer"><Sparkles size={16} className="text-yellow-600" /> <span className="text-sm">IA Notion</span></div>
                    <div className="flex items-center gap-3 p-1.5 rounded hover:bg-zinc-200 text-zinc-600 cursor-pointer"><Inbox size={16} /> <span className="text-sm">Bandeja</span></div>
                    
                    <div className="mt-4 mb-1 px-2 flex justify-between items-center group"><span className="text-xs font-medium text-zinc-500">PRIVADO</span><button onClick={(e) => { e.stopPropagation(); createPage(); }} className="text-zinc-400 hover:text-zinc-600 opacity-0 group-hover:opacity-100 transition-opacity"><Plus size={12}/></button></div>
                    {workspace.pages.map(page => <div key={page.id} onClick={() => { setActivePageId(page.id); setCurrentView('page'); }} className={`flex items-center gap-3 p-1.5 rounded hover:bg-zinc-200 cursor-pointer group ${activePageId === page.id ? 'bg-zinc-200 text-zinc-900 font-medium' : 'text-zinc-600'}`}><span className="text-lg leading-none w-4 text-center shrink-0">{page.icon || <FileText size={16} />}</span><span className="text-sm truncate opacity-90">{page.title || 'Sin título'}</span></div>)}
                    <div onClick={createPage} className="flex items-center gap-3 p-1.5 rounded hover:bg-zinc-200 text-zinc-500 cursor-pointer mt-2"><Plus size={16} /> <span className="text-sm">Añadir página</span></div>
                </div>

                <div className="p-2 border-t border-zinc-200 space-y-0.5">
                    <div onClick={() => { setCurrentView('settings'); setSettingsTab('menu'); }} className="flex items-center gap-3 p-1.5 rounded hover:bg-zinc-200 text-zinc-600 cursor-pointer"><Settings size={16} /> <span className="text-sm">Config</span></div>
                    <div onClick={() => { setShowMarket(true); setMarketTab('styles'); }} className="flex items-center gap-3 p-1.5 rounded hover:bg-zinc-200 text-zinc-600 cursor-pointer"><Palette size={16} /> <span className="text-sm">Styles Market</span></div>
                    <div onClick={() => setCurrentView('trash')} className="flex items-center gap-3 p-1.5 rounded hover:bg-zinc-200 text-zinc-600 cursor-pointer"><Trash size={16} /> <span className="text-sm">Papelera</span></div>
                    
                    <div onClick={() => setSidebarOpen(false)} className="flex items-center gap-3 p-1.5 rounded hover:bg-zinc-200 text-zinc-600 cursor-pointer"><ChevronsLeft size={16} /> <span className="text-sm">Colapsar</span></div>
                </div>
            </motion.aside>
                        ...
                        <div className="max-w-2xl mx-auto pt-4">
                            {settingsTab === 'menu' && (
                                <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}>
                                    <h1 className="text-xl font-bold mb-6 text-zinc-800">Configuración</h1>
                                    <div className="bg-zinc-50 rounded-xl border border-zinc-200 overflow-hidden">
                                        <div className="p-4 hover:bg-zinc-100 cursor-pointer flex justify-between items-center border-b border-zinc-200"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-zinc-200 flex items-center justify-center text-zinc-500"><User size={16}/></div><div><div className="font-medium text-sm">Cuenta</div><div className="text-xs text-zinc-500">Gestiona tu perfil</div></div></div><ArrowRight size={16} className="text-zinc-400"/></div>
                                        <div onClick={() => setSettingsTab('appearance')} className="p-4 hover:bg-zinc-100 cursor-pointer flex justify-between items-center border-b border-zinc-200"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-zinc-200 flex items-center justify-center text-zinc-500"><Palette size={16}/></div><div><div className="font-medium text-sm">Apariencia</div><div className="text-xs text-zinc-500">Temas y colores</div></div></div><ArrowRight size={16} className="text-zinc-400"/></div>
                                        <div onClick={() => setSettingsTab('fonts')} className="p-4 hover:bg-zinc-100 cursor-pointer flex justify-between items-center"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-zinc-200 flex items-center justify-center text-zinc-500"><Type size={16}/></div><div><div className="font-medium text-sm">Tipografía</div><div className="text-xs text-zinc-500">Fuentes y tamaño</div></div></div><ArrowRight size={16} className="text-zinc-400"/></div>
                                    </div>
                                </motion.div>
                            )}
                            {(settingsTab === 'appearance' || settingsTab === 'fonts') && (
                                <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }}>
                                    <button onClick={() => setSettingsTab('menu')} className="flex items-center gap-1 text-zinc-500 mb-4 text-sm hover:text-zinc-800"><ArrowLeft size={14}/> Volver</button>
                                    <h2 className="text-xl font-bold mb-6 text-zinc-800">{settingsTab === 'appearance' ? 'Apariencia' : 'Tipografía'}</h2>
                                    <div className="space-y-2">
                                        {(settingsTab === 'appearance' ? importedResources.styles : importedResources.fonts).length === 0 ? <p className="text-zinc-500 text-xs">No hay recursos importados.</p> : (settingsTab === 'appearance' ? importedResources.styles : importedResources.fonts).map(r => (<div key={r.id} className="bg-white p-3 rounded-lg border border-zinc-200 flex justify-between items-center shadow-sm"><div className="flex items-center gap-3"><div className="w-8 h-8 bg-zinc-100 rounded border border-zinc-200"></div><div><div className="font-medium text-sm">{r.name}</div><div className="text-xs text-zinc-400">Importado</div></div></div><button className="text-indigo-600 text-xs font-medium">Aplicar</button></div>))}
                                    </div>
                                </motion.div>
                            )}
                        </div>
            ...
            {/* SEARCH */}
            <Modal isOpen={showSearch} onClose={() => setShowSearch(false)} title={null}>
                <div className="p-4 border-b border-zinc-100 bg-[#FBFBFA]"><div className="flex items-center gap-3 bg-white px-3 py-2.5 rounded-lg mb-3 border border-zinc-200 shadow-sm"><Search className="text-zinc-400" size={20} /><input autoFocus value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Buscar páginas..." className="bg-transparent w-full outline-none text-lg placeholder:text-zinc-400 text-zinc-700" /></div></div>
                {searchQuery ? <div className="p-1 bg-[#FBFBFA] min-h-[300px]">{filteredPages.map(p => <div key={p.id} onClick={() => { setActivePageId(p.id); setCurrentView('page'); setShowSearch(false); }} className="flex items-center gap-3 p-2 mx-2 hover:bg-zinc-200 rounded cursor-pointer group"><div className="text-lg">{p.icon || '📄'}</div><div><div className="text-sm font-medium text-zinc-700">{p.title || 'Sin título'}</div></div></div>)}</div> : <div className="h-[300px] flex items-center justify-center text-zinc-400 text-sm">Empieza a escribir para buscar...</div>}
            </Modal>

            {/* AI */}
            <Modal isOpen={showAI} onClose={() => setShowAI(false)} title={null} transparent={true}>
                <div className="flex items-center justify-center h-full p-4">
                    <div className="bg-white/80 backdrop-blur-xl border border-white/50 rounded-2xl shadow-2xl p-6 w-full max-w-lg relative overflow-hidden min-h-[300px] flex flex-col">
                        <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/10 to-purple-500/10 pointer-events-none"/>
                        {isAiGenerating ? (
                            <div className="flex-1 flex items-center justify-center"><BreathingText text="Creando Ideas..." /></div>
                        ) : (
                            <div className="relative z-10 flex-1 flex flex-col">
                                <h2 className="text-2xl font-bold text-zinc-800 mb-2 flex items-center gap-2"><Sparkles className="text-indigo-500"/> IA Notion</h2>
                                <p className="text-zinc-500 text-sm mb-6">Describe la página que quieres crear y la IA se encargará del resto.</p>
                                <textarea value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} placeholder="Ej: Una lista de tareas para organizar un viaje a Japón..." className="w-full bg-white border border-zinc-200 rounded-xl p-4 h-32 resize-none outline-none focus:ring-2 focus:ring-indigo-500/20 mb-4 text-zinc-700 placeholder:text-zinc-400"/>
                                <div className="flex justify-end gap-2 mt-auto">
                                    <button onClick={() => setShowAI(false)} className="px-4 py-2 rounded-lg text-zinc-500 hover:bg-zinc-100 transition-colors text-sm">Cancelar</button>
                                    <button onClick={generatePageWithAI} disabled={!aiPrompt.trim() || isAiGenerating} className="px-6 py-2 rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-medium text-sm hover:shadow-lg hover:scale-105 transition-all disabled:opacity-50 disabled:scale-100 flex items-center gap-2"><Wand2 size={16}/> Generar Página</button>
                                </div>
                            </div>
                        )}
                    </div>
            ...
            <Modal isOpen={showMarket} onClose={() => setShowMarket(false)} title={null} size="max-w-4xl">
                <div className="flex flex-col h-full bg-[#FBFBFA]">
                    <div className="flex justify-between items-center px-6 pt-6 pb-2"><div className="flex gap-6 text-sm font-medium">{['Galería', 'Estilos', 'Fuentes'].map(tab => <button key={tab} onClick={() => setMarketTab(tab.toLowerCase())} className={`pb-2 px-1 transition-colors relative ${marketTab === tab.toLowerCase() ? 'text-indigo-600' : 'text-zinc-500 hover:text-zinc-800'}`}>{tab}{marketTab === tab.toLowerCase() && <motion.div layoutId="market-tab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-indigo-600"/>}</button>)}</div><button onClick={() => setShowMarket(false)} className="text-zinc-400 hover:text-zinc-600"><X size={20}/></button></div>
                    <div className="h-px bg-zinc-200 mx-6 mb-6"/>
                    <div className="flex-1 overflow-y-auto px-6 pb-6 min-h-[400px]">
                        {isLoadingMarket ? <div className="flex flex-col items-center justify-center h-full text-zinc-400 gap-3"><Loader2 className="animate-spin text-indigo-400" size={32}/><span>Conectando con el repositorio...</span></div> : (
                            <>
                                {marketTab === 'galería' && <div className="space-y-8"><div className="space-y-4"><h4 className="text-xs font-bold text-zinc-400 uppercase">LOCALES</h4><div className="grid grid-cols-2 md:grid-cols-4 gap-4">{Object.values(MOCK_COVERS).flat().map((url, i) => <div key={i} onClick={() => { updatePage(activePageId, { cover: url }); setShowMarket(false); }} className="h-24 rounded-xl overflow-hidden cursor-pointer border-2 border-transparent hover:border-indigo-500 transition-all shadow-sm"><img src={url} className="w-full h-full object-cover"/></div>)}</div></div>{marketData.covers['Temas del Repo'] && <div className="space-y-4"><h4 className="text-xs font-bold text-zinc-400 uppercase">DEL REPOSITORIO</h4><div className="grid grid-cols-2 md:grid-cols-4 gap-4">{marketData.covers['Temas del Repo'].map((url, i) => <div key={i} onClick={() => { updatePage(activePageId, { cover: url }); setShowMarket(false); }} className="h-24 rounded-xl overflow-hidden cursor-pointer border-2 border-transparent hover:border-indigo-500 transition-all shadow-sm"><img src={url} className="w-full h-full object-cover"/></div>)}</div></div>}</div>}
                                {marketTab === 'estilos' && <div className="grid grid-cols-1 md:grid-cols-2 gap-6">{marketData.styles.map(s => <div key={s.id} className="bg-white rounded-xl overflow-hidden shadow-sm border border-zinc-100 flex flex-col"><div className={`h-32 ${s.color} flex items-center justify-center text-4xl font-bold opacity-40`}>Aa</div><div className="p-4"><h3 className="font-bold text-zinc-800">{s.name}</h3><p className="text-xs text-zinc-500 mb-4">Por {s.author}</p><button onClick={() => importResource(s, 'style')} className="w-full py-2 bg-indigo-600 text-white rounded-lg text-xs font-medium hover:bg-indigo-700 transition-colors">Obtener</button></div></div>)}</div>}
                                {marketTab === 'fuentes' && <div className="grid gap-4">{marketData.fonts.map(f => <div key={f.id} className="bg-white p-4 rounded-xl border border-zinc-200 flex justify-between items-center shadow-sm"><div><div className="text-xl font-serif mb-1 text-zinc-800">{f.name}</div><div className="text-xs text-zinc-500">Fuente Open Source</div></div><button onClick={() => importResource(f, 'font')} className="bg-zinc-100 px-4 py-2 rounded-lg text-xs font-medium text-zinc-600 hover:bg-zinc-200">Obtener</button></div>)}</div>}
                            </>
                        )}
                    </div>
                </div>
