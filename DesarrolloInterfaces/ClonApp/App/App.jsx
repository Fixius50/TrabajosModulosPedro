...
import { 
    ...
    Wand2, Loader2, LayoutTemplate, Bell, ChevronsLeft, Menu
} from 'lucide-react';
...
// POR DEFECTO: Sin páginas para mostrar el estado vacío
...
// Texto animado en ola (Rebotando letras)
const WavyText = ({ text }) => {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      className="text-zinc-400 text-sm font-medium mt-4"
    >
...
const BreathingText = ({ text }) => (
    <div className="flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-sm text-zinc-500 font-medium">Contactando con tu API en Vercel...</p>
    </div>
);
    ...
    <AnimatePresence>
        {isVisible && (
            ...
            >
                ...
                <button onClick={onClose} className="bg-white text-black px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-zinc-200 transition-colors shadow-sm">Aceptar</button>
            </motion.div>
        )}
    </AnimatePresence>
);
...
const EditorBlock = ({ block, index, updateBlock, addBlock, deleteBlock }) => {
    ...
    return (
        <motion.div layout className="group relative flex items-start pl-6 mb-1">
            <div className="absolute left-0 top-1.5 p-1 text-zinc-300 opacity-0 group-hover:opacity-100 cursor-grab active:cursor-grabbing hover:text-zinc-500"><GripVertical size={16} /></div>
 ...
 * Exportado como default para que index.html lo importe
 */
export default function App() {
    const [workspaces, setWorkspaces] = useState(() => { const saved = localStorage.getItem('notion_v49_workspaces'); return saved ? JSON.parse(saved) : INITIAL_WORKSPACES; });
    const [sidebarOpen, setSidebarOpen] = useState(true); // Por defecto abierta
    ...
    useEffect(() => { localStorage.setItem('notion_v49_workspaces', JSON.stringify(workspaces)); }, [workspaces]);
    ...
    // AI Generator (Conectado a Vercel backend)
    const generatePageWithAI = async () => {
        if (!aiPrompt.trim()) return;
        setIsAiGenerating(true);
        try {
            // Llamada relativa al backend en Vercel (mismo dominio)
            const response = await fetch('/api/generar-pagina', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ prompt: `Eres un diseñador experto de Notion. Crea un JSON para: "${aiPrompt}". JSON: { "title": "str", "icon": "emoji", "cover_keyword": "english_keyword", "blocks": [{"type": "h1"|"p"|"todo", "content": "str"}] }` })
            });
            ...
            const newPage = {
                id: generateId(), title: parsed.title || 'Idea IA', icon: parsed.icon || '✨', cover: dynamicCover, updatedAt: new Date().toISOString(), isPrivate: true, blocks: parsed.blocks?.map(b => ({ ...b, id: generateId() })) || []
            };
            ...
            setShowAI(false);
            setAiPrompt("");
            setNotification({ show: true, message: `"${newPage.title}" ha sido añadida.` });

        } catch (e) {
            console.error("AI Error:", e);
            alert("Error conectando con la IA. Verifica que el backend está corriendo.");
            ...
            <main className="flex-1 h-full overflow-hidden relative flex flex-col bg-white">
                <header className="h-12 flex items-center justify-between px-3 shrink-0 hover:bg-transparent transition-colors">
                    <div className="flex items-center gap-2 text-sm text-zinc-600 overflow-hidden w-full">
                        {!sidebarOpen && <button onClick={() => setSidebarOpen(true)} className="p-1 hover:bg-zinc-100 rounded mr-2 md:hidden"><Menu size={18}/></button>}
