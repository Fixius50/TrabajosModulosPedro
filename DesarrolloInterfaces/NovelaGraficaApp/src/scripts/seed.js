...
async function seed() {
    ...
    const { data: series, error: seriesError } = await supabase
        .from('series')
        .insert({
            title: 'Demos Técnicas ' + new Date().toISOString().slice(0, 10),
    ...
    const dialogues = [
        ...
        { node_id: stealthNode.id, speaker_name: 'Narrador', content: 'Avanzas entre los árboles de código binario sin hacer ruido. Ves una luz azul a lo lejos.', position_y: 85 },
        ...
        { node_id: endNode.id, speaker_name: '', content: 'Continuará...', position_y: 50 },
    ];
    ...
    console.log("✅ Choices connected for Demo.");
    ...
    const createStory = async (title, desc, genre, coverUrl, nodesData) => {
        ...
        // 2. Chapter
        const { data: c } = await supabase.from('chapters').insert({ series_id: s.id, title: 'Capítulo 1', order_index: 1 }).select().single();
    ...
    await createStory(
        ...
        'Tu grupo ha caído en una emboscada.',
        'Fantasía',
        '/assets/images/cover_dnd.png',
        [
            /* 0 */ { text: "El goblin ríe mientras bloquea la salida. Tu espada pesa en tu mano.", speaker: "DM", choices: [{ label: "Atacar", targetIndex: 1 }, { label: "Dialogar", targetIndex: 2 }] },
            /* 1 */ { text: "¡Crítico! Cortas la cuerda del puente y el goblin cae al vacío.", speaker: "DM", nextIndex: 3 },
    ...
    await createStory(
        ...
        [
            /* 0 */ { text: "La lluvia cae sobre la gárgola. La señal brilla en el cielo.", speaker: "Batman", choices: [{ label: "Ir al Muelle", targetIndex: 1 }, { label: "Ir al Banco", targetIndex: 2 }] },
            /* 1 */ { text: "Harley Quinn te espera con un mazo gigante.", speaker: "Alfred", nextIndex: 3 },
            /* 2 */ { text: "Es una trampa. Dos Caras ha bloqueado la bóveda.", speaker: "Oráculo", nextIndex: 3 },
    ...
    await createStory(
        ...
        'Entrar y salir, Morty. Rápido.',
        'Sci-Fi',
        '/assets/images/cover_rick.png',
        [
            /* 0 */ { text: "Morty, tienes que meterte estas semillas... ya sabes dónde.", speaker: "Rick", choices: [{ label: "¡Oh jeez, Rick!", targetIndex: 1 }, { label: "Ni hablar", targetIndex: 2 }] },
            /* 1 */ { text: "Bien hecho Morty. Ahora corre, la Federación Galáctica está aquí.", speaker: "Rick", nextIndex: 3 },
            /* 2 */ { text: "No seas aguafiestas Morty. Mira, ahora nos disparan.", speaker: "Rick", nextIndex: 3 },
            /* 3 */ { text: "Portal gun sin carga. Toca correr en círculos. Wubba Lubba Dub Dub!", speaker: "Rick", type: 'ending' }
        ]
    );
    ...
    if (authError) {
        console.log(`⚠️ User creation note: ${authError.message}`);
        // Likely 'User already registered', which is fine.
    } else {
        console.log(`✅ User Sign-up request sent for ${email}.`);
